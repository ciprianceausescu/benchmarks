...
class LarsOptimizer(optimizer.Optimizer):
  ...
  def _apply_dense(self, grad, var):
    # Variable grad = stochastic gradient
    # Variable var = weights
    # get_slot method from Optimizer class returns the "momentum" slot created for weigths variable
    momentum = self.get_slot(var, "momentum")
    # Calculate the L2-norm for weights
    weight_norm = tf.sqrt(tf.reduce_sum(tf.square(var)))
    # Calculate the L2-norm for stochastic gradient
    gradient_norm = tf.sqrt(tf.reduce_sum(tf.square(grad)))
    # Compute the local learning rate
    local_lr = self._nu * weight_norm / (gradient_norm + self._beta * weight_norm)
    # Update the momentum variable
    momentum_update = self._momentum * momentum + self._learning_rate * local_lr * (grad+self._beta*var)
    # Update the weights variable
    var_update = var - momentum_update
    # Update the parameter - momentum. Assign to momentum variable the new value - momentum_update
    update_params_op = state_ops.assign(momentum, momentum_update)
    # Update the parameter - weigths. Assign to weights variable the new value - var_update
    update_memory_op = state_ops.assign(var, var_update)
...
class BenchmarkCNN(object):
  ...
  def __init__(self, params, dataset=None, model=None):
    ...
    # Added self.params to get_model_config
    ...
    #self.use_synthetic_gpu_images = self.dataset.use_synthetic_gpu_inputs()
